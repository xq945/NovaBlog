...
开发新功能前，必须先编写设计文档，明确接口设计、数据库变更、核心逻辑、安全考虑和前端交互。文档存放于 `design/` 目录，经用户确认后方可进入编码阶段。模板见 [design/template.md](./design/template.md)。
...
| 周次 | 内容 | 产出 |
|------|------|------|
| 第一周 | 项目初始化 + 用户模块 | 前后端项目跑通，注册登录功能完成 |
| 第二周 | JWT 认证 + 文章模块 | 登录带 Token，文章 CRUD 完成 |
| 第三周 | 评论系统 + Redis | 评论嵌套显示，点赞/浏览量功能完成 |
| 第四周 | 文件上传 + 个人中心 | 头像上传，个人信息修改完成 |
| 第五周 | 后台管理 + 安全加固 | 管理员功能完成，安全规范落地 |
| 第六周 | 部署上线 | Docker 化，Nginx 部署，线上可访问 |

## 相关文档

- [CLAUDE.md](./CLAUDE.md) — 详细开发指南（数据库设计、Redis Key 规范、UI 设计规范、可用技能）
- [Blog全栈开发路线图](./Blog全栈开发路线图(前后端联调版).md) — 按阶段细分的完整开发路线图
